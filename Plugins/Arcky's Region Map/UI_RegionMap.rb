...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(as_editor = false, fly_map = false)
    ...
    if @mode == 1
      ...
      @flyIconsPositions.each do |point|
        @sprites["point#{point[2]}"] = IconSprite.new(0, 0, @viewport)
        iconName = @visitedMaps.find { |name| point[2] == name[2] } ? "mapFly" : "mapFlyDis"
        @sprites["point#{point[2]}"].setBitmap("Graphics/Pictures/RegionMap/Icons/#{iconName}")
        @sprites["point#{point[2]}"].x        = point_x_to_screen_x(point[0])
        @sprites["point#{point[2]}"].y        = point_y_to_screen_y(point[1])
        @sprites["point#{point[2]}"].z        = 50
      end
    end
    if !@sprites["Visited"]
    ...
    maps = RegionMapSettings::USE_UNVISITED_FOLDER ? @unvisitedMaps : @visitedMaps
    folder = RegionMapSettings::USE_UNVISITED_FOLDER ? "Unvisited" : "Visited"
    maps.each do |visit|
      @sprites["Visited"].z = 6 #the highlighted City/Town gets drawn over the locations as well.
      pbDrawImagePositions(
        @sprites["Visited"].bitmap,
        [["Graphics/Pictures/RegionMap/#{folder}/map#{visit[8]}", ((visit[0] - 1) * SQUARE_WIDTH) , ((visit[1] - 1) * SQUARE_HEIGHT)]]
      )
    end
    ...
    @questMap.each do |index|
      ...
      @sprites["mapQuest#{index}"].visible  = @showQuests == true && index[0] == @playerpos[0]
    ...
    @sprites["cursor"].viewport = @Viewport2
    @sprites["cursor"].x        = point_x_to_screen_x(@map_x)
    @sprites["cursor"].y        = point_y_to_screen_y(@map_y)
    @sprites["cursor"].z        = 999999
    @sprites["cursor"].play
    @sprites["help"] = BitmapSprite.new(Graphics.width, 32, @Viewport2)
  ...
  def recalculateFlyIconPositions
    centerHash = {}
    matchingElements = []
    @flyIconsPositions = @mode == 1 ? @visitedMaps.map(&:dup) + @unvisitedMaps.map(&:dup) : []
    return if @mode != 1
    @flyIconsPositions.each do |element|
      mapName = element[2]
      # Skip processing if this mapName has already been processed.
  ...
  def pbGetMapLocation(x, y)
    ...
    @map[2].each do |point|
      ...
      if @mapSize[1].any? { |item| item.include?("Route") }
        ...
        @mapSize[0] = minValues
        colorCurrentLocation(nil, point)
      end
  ...
  def colorCurrentLocation(mapSize = nil, point = [])
    ...
    if @mode != 1
      if !@mapSize[1][0].include?("Route")
        ...
        @mapSize[0][0] -= 1 if !@mapSize[1][0].include?("S")
        @mapSize[0][1] -= 1 if !@mapSize[1][0].include?("S")
        pbDrawImagePositions(
          @sprites["highlight"].bitmap,
          [["Graphics/Pictures/RegionMap/Highligths/#{mapFolder}/map#{@mapSize[1][0]}", ((@mapSize[0][0]) * SQUARE_WIDTH) , ((@mapSize[0][1]) * SQUARE_HEIGHT)]]
        )
      else 
        ...
        pbDrawImagePositions(
          @sprites["highlight"].bitmap,
          [["Graphics/Pictures/RegionMap/Highligths/Routes/#{@tileType}", ((mapSize[0]) * SQUARE_WIDTH) , ((mapSize[1]) * SQUARE_HEIGHT)]]
        )
      end
    else  # we have to check and only highlight the maps that are included in @flyMapPositions
      flyMap = @flyIconsPositions.find { |map| map[2] == @mapSize[2][0] }
      return if !flyMap || @unvisitedMaps.find { |name| flyMap[2] == name[2] }
      @sprites["highlight"].z = 51 #the hightlighted Routes gets drawn over the Route Region map but stay below the City and Location Region Map.
        pbDrawImagePositions(
          @sprites["highlight"].bitmap,
  ...
  def pbMapScene
    ...
    loop do
      ...
      if x_offset == 0 && y_offset == 0 && choice && choice >= 0
      ...
      if Input.trigger?(Input::JUMPUP) && @mode == 1
        ...
        if choice != -1 && @visitedMaps[choice][2] != pbGetMapLocation(@map_x, @map_y)
          ox = @visitedMaps[choice][0] - @map_x if @map_x <= RIGHT && @map_x >= LEFT 
          oy = @visitedMaps[choice][1] - @map_y if @map_y <= BOTTOM && @map_y >= TOP
        end
      end
      if ox != 0 || oy != 0
        @map_x += ox
      ...
      elsif (Input.trigger?(Input::USE) && @mode == 1) || inputFly # Choosing an area to fly to
...
# Wall Region Map
#===============================================================================
def pbShowMap(region = -1, wallmap = true, quest = false)
  pbFadeOutIn {
    scene = PokemonRegionMap_Scene.new(region, wallmap, quest)
...
MenuHandlers.add(:pokegear_menu, :map, {
  ...
  "effect"    => proc { |menu|
    pbFadeOutIn {
      scene = PokemonRegionMap_Scene.new(-1, false, true)
      screen = PokemonRegionMapScreen.new(scene)
      ret = screen.pbStartScreen
      if ret
...
#===============================================================================
# Region Map Item from the Ready Menu
#===============================================================================
ItemHandlers::UseInField.add(:TOWNMAP, proc { |item|
  pbShowMap(-1, false, true) if $game_temp.fly_destination.nil?
  pbFlyToNewLocation
  next true
})
#===============================================================================
